...
```{sql connection=con}
SELECT PNAME, ACRES, ACRES * 1.20 AS NEW_ACRES FROM PRESERVE
```
...
```{sql connection=con}
SELECT PNAME, ACRES, ACRES/3.0 AS ACRES_MIN FROM PRESERVE
```
...
```{sql connection=con}
SELECT PNAME, FEE, (FEE + 50.0)/2 AS FEE_NEW FROM PRESERVE 
```
...
```{sql connection=con}
SELECT SUBSTR(PNAME, 1, INSTR(PNAME || ' ', ' ') - 1) AS primeiro_nome
FROM PRESERVE
...
```{sql connection=con}
SELECT PNAME, COUNT(*) AS QUANTIDADE FROM PRESERVE
GROUP BY PNAME
HAVING COUNT(*) > 1;
```
...
```{sql connection=con}
SELECT AVG(ACRES * 0.02) AS taxa_media_arizona FROM PRESERVE
WHERE STATE = 'AZ'
```
...
```{sql connection=con}
SELECT ENO, ENAME, SALARY AS OLD_SALARY, SALARY * 1.10 AS NEW_SALARY
FROM EMPLOYEE
ORDER BY NEW_SALARY;
```
